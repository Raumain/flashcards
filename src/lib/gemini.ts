...
export class GeminiError extends Error {
  constructor(
    public code: "API_KEY_MISSING" | "RATE_LIMITED" | "CONTENT_FILTERED" | "GENERATION_FAILED" | "TIMEOUT",
    message: string,
...
function buildMessageContent(images: { base64: string; mimeType: "image/jpeg" }[]) {
  const content: Array<{ type: "text"; text: string } | { type: "image"; image: string; mimeType: "image/jpeg" }> = [];
...
export async function generateFlashcardsFromImages(
  ...
  if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
    ...
    throw new GeminiError("API_KEY_MISSING", "GOOGLE_GENERATIVE_AI_API_KEY environment variable is not set");
  }
  ...
  try {
    ...
    log("info", `Generation completed in ${duration}s - ${result.object.flashcards?.length ?? 0} flashcards`);
    ...
    if (error instanceof Error) {
      if (error.message.includes("RATE_LIMIT")) {
        throw new GeminiError("RATE_LIMITED", "API rate limit exceeded. Please try again later.", error);
      }
 ...
 */
export async function streamFlashcardsFromImages(images: { base64: string; mimeType: "image/jpeg" }[]) {
  const startTime = Date.now();
 ...
 */
export async function awaitStreamResult(
  streamData: { result: ReturnType<typeof streamObject<typeof GenerationResultSchema>>; startTime: number },
): Promise<GenerationResult> {
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    timeoutId = setTimeout(() => {
      isTimedOut = true;
      reject(new GeminiError("TIMEOUT", `AI generation timed out after ${GENERATION_TIMEOUT_MS / 1000}s`));
