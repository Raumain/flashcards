...
/**
 * Safely unlinks a file after validating:
 * 1. It's within the expected base directory
 * 2. It's not a symlink (prevents symlink attacks)
 */
async function safeUnlink(filePath: string, basePath: string): Promise<void> {
	// Validate path is within expected directory
	if (!isPathWithinBase(filePath, basePath)) {
		console.warn(
			`[pdf-processor] Security: Refusing to unlink path outside base directory: ${filePath}`,
		)
		return
	}

	try {
		// Check if file is a symlink - refuse to delete symlinks
		const stats = await lstat(filePath)
		if (stats.isSymbolicLink()) {
			console.warn(`[pdf-processor] Security: Refusing to unlink symlink: ${filePath}`)
			return
		}
...
function validatePDF(buffer: ArrayBuffer): void {
	...
	if (!isValidPDF) {
		throw new PDFProcessingError('INVALID_PDF', 'File does not appear to be a valid PDF')
	}
}
...
async function runPdftoppm(
	...
	return new Promise((resolve, reject) => {
		const args = [
			...
			'-l',
			String(options.maxPages), // last page
			pdfPath,
			outputPrefix,
		]
		...
		proc.on('error', (err) => {
			reject(
				new PDFProcessingError('CONVERSION_FAILED', `Failed to spawn pdftoppm: ${err.message}`),
			)
		})
	})
}
...
export async function pdfToImages(
	...
	console.log(
		`[pdf-processor] PDF validation passed, size: ${(pdfBuffer.byteLength / 1024 / 1024).toFixed(2)}MB`,
	)
	...
	try {
		...
		console.log(
			`[pdf-processor] pdftoppm completed in ${Date.now() - pdftoppmStart}ms - ${pngFiles.length} pages`,
		)
		...
		for (let i = 0; i < pngFiles.length; i += BATCH_SIZE) {
			...
			console.log(
				`[pdf-processor] Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(pngFiles.length / BATCH_SIZE)}`,
			)
			...
			const batchResults = await Promise.all(
				batch.map(async (pngPath, batchIndex) => {
					...
					// Use safeUnlink to prevent symlink attacks
					try {
						await safeUnlink(pngPath, tempDir)
					} catch {
		...
		// Using rm with recursive: true is safer than iterating files,
		// as it handles the directory as a unit and avoids symlink attacks
		try {
			// Validate tempDir is actually in system temp directory
			if (!isPathWithinBase(tempDir, tmpdir())) {
				console.error(
					`[pdf-processor] Security: Refusing to delete directory outside tmpdir: ${tempDir}`,
				)
			} else {
