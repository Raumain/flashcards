 ...
 */
function getImageDataUrl(pageImages: PageImage[] | undefined, pageIndex: number | undefined): string | null {
...
async function calculateBackPageHeight(
...
): Promise<{ height: number; imageWidth: number; imageHeight: number; imageDataUrl: string | null }> {
  ...
  if (card.back.details) {
    doc.setFontSize(10);
    const detailLines = wrapText(doc, card.back.details, imageDataUrl ? leftColumnWidth : contentWidth);
...
function drawCardBack(
  ...
  if (hasImage) {
    try {
      ...
      doc.addImage(imageDataUrl, "JPEG", rightColumnX, imageY, imageWidth, imageHeight, undefined, "MEDIUM");
...
function drawCardBackPrintable(
  ...
  if (card.back.details && currentY < y + height - 15) {
    ...
    detailLines.slice(0, Math.floor((y + height - currentY - 5) / detailHeight)).forEach((line, i) => {
...
export async function generateDigitalPDF(
  ...
  return generateFlashcardPDF(flashcards, {
    ...config,
    pageSize: "a5",
    orientation: "landscape",
  }, pageImages);
